...
new_step "Intel install"
    sub_step "wget -qO- https://apt.repos.intel.com/intel-gpg-keys/GPG-PUB-KEY-INTEL-SW-PRODUCTS.PUB | gpg --dearmor --output /usr/share/keyrings/intel-oneapi-archive-keyring.gpg"
    wget -qO- https://apt.repos.intel.com/intel-gpg-keys/GPG-PUB-KEY-INTEL-SW-PRODUCTS.PUB | gpg --dearmor --output /usr/share/keyrings/intel-oneapi-archive-keyring.gpg
    sub_step "echo 'deb [signed-by=/usr/share/keyrings/intel-oneapi-archive-keyring.gpg] https://apt.repos.intel.com/oneapi all main' > /etc/apt/sources.list.d/oneAPI.list"
...
new_step "Ubuntu tools for package management"
    ...
    sub_step "dpkg -l  > $TARGET_DIR/installed-with-versions.txt"
